...
sj_model = RandomForestRegressor(n_estimators=200, max_depth=6, criterion='mae', warm_start=True)
sj_model.fit(sj_x_train, sj_y_train)
sj_predict_value = sj_model.predict(sj_x_test)


iq_model = RandomForestRegressor(n_estimators=20, max_features='auto', max_depth=6, min_samples_leaf=0.005,
                                 criterion='mae', min_weight_fraction_leaf=0.1, warm_start=True)
